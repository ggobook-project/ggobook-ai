class PromptUtil:
    ...
    @staticmethod
    def get_format_dialogue_prompt(text: str) -> str:
        return f"""당신은 한국어 소설 편집 AI입니다.
아래 소설 원문에서 등장인물이 직접 말하는 대사 부분을 감지하고, 해당 대사를 큰따옴표("")로 정확히 감싸주세요.

규칙:
- 대사(인물이 직접 말하는 부분)는 반드시 큰따옴표("")로 감싸세요
- 나레이션(서술 부분)은 절대 수정하지 마세요
- 이미 큰따옴표로 감싸진 대사는 그대로 유지하세요
- 줄바꿈, 문단 구조를 원문 그대로 유지하세요
- 추가 설명, 주석, 마크다운 없이 변환된 텍스트만 출력하세요
